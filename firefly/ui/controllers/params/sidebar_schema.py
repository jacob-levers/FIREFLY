...
_TOOLTIPS = {
    ...
    "analysis/minmass_mode": "How the auto threshold is chosen. Linkability tunes each file on its own. Density-matched instead gives every file the same detections per frame, so the threshold can't differ between conditions and masquerade as a biological difference.",
