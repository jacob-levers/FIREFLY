...
_TOOLTIPS = {
    ...
    "analysis/minmass_sensitivity": "How aggressive the auto-threshold is. Strict = fewer false spots; Lenient = more detections. (Linkability mode only.)",
...
FIELDS = [
    ...
    _f("detection", "analysis/min_cnr", "double", "Min raw contrast / noise", 0.0,
       min=0.0, max=20.0, step=0.5, decimals=1, special="off",
       tooltip="Reject weak raw-image peaks using central 3×3 contrast over annular pixel noise. Independent of frame normalization. Not a false-positive probability. Requires raw images; inspect overlays before selecting a cutoff."),
    ...
    _f("drift", "analysis/drift_method", "combo", "Drift method", "RCC (density maps)",
       items=["RCC (density maps)", "Stationary fiducials"],
       tooltip="Fiducials must be stationary markers selected in a separate reference region or CSV."),
    _f("drift", "analysis/drift_adaptive", "bool", "Adapt windows to signal", True,
       tooltip="Shorten populated windows and extend sparse ones, between roughly ¼ and 4× the segment length. At most 64 windows."),
    _f("drift", "analysis/drift_min_locs", "int", "Min reference spots / window", 200,
       min=5, max=100000, step=50),
    _f("drift", "analysis/drift_min_correlation", "double", "Min peak correlation", 0.2,
       min=0.0, max=1.0, step=0.05, decimals=2,
       tooltip="Reject weak density correlations. This is a diagnostic threshold, not a probability."),
    _f("drift", "analysis/drift_reference", "combo", "Drift reference", "Analysis region",
       items=["Analysis region", "Separate rectangle", "Reference CSV"],
       tooltip="Estimate drift independently of the tracking ROI. Rectangle uses raw camera pixel coordinates."),
    _f("drift", "analysis/drift_ref_x", "double", "Reference left (px)", 0., min=0., max=100000., step=1., decimals=1,
       enable={"key": "analysis/drift_reference", "eq": "Separate rectangle"}),
    _f("drift", "analysis/drift_ref_y", "double", "Reference top (px)", 0., min=0., max=100000., step=1., decimals=1,
       enable={"key": "analysis/drift_reference", "eq": "Separate rectangle"}),
    _f("drift", "analysis/drift_ref_width", "double", "Reference width (px)", 64., min=1., max=100000., step=1., decimals=1,
       enable={"key": "analysis/drift_reference", "eq": "Separate rectangle"}),
    _f("drift", "analysis/drift_ref_height", "double", "Reference height (px)", 64., min=1., max=100000., step=1., decimals=1,
       enable={"key": "analysis/drift_reference", "eq": "Separate rectangle"}),
    _f("drift", "analysis/drift_reference_file", "file", "Reference localisations CSV", "",
       enable={"key": "analysis/drift_reference", "eq": "Reference CSV"},
       tooltip="Same acquisition and frame clock: zero-based frame, x, y in camera pixels; optional particle IDs for fiducials. {stem} expands per movie in a batch."),
    _f("drift", "analysis/drift_min_fiducials", "int", "Min shared fiducials", 3, min=1, max=100, step=1,
       enable={"key": "analysis/drift_method", "eq": "Stationary fiducials"},
       tooltip="Three or more markers provide disagreement checks. One marker cannot establish its own stability."),
    _f("drift", "analysis/drift_fiducial_range", "double", "Fiducial link range (px)", 2., min=.1, max=50., step=.1, decimals=1,
       enable={"key": "analysis/drift_method", "eq": "Stationary fiducials"},
       tooltip="Used only when reference detections lack particle IDs. Gap memory is two frames."),
    _f("drift", "analysis/drift_failure_policy", "combo", "Unsupported drift", "Skip correction",
       items=["Skip correction", "Stop run"],
       tooltip="An unsupported estimate applies no correction and records a QC warning, or stops the run. No silent interpolation over disconnected references."),
